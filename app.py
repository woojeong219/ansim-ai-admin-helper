...
with excel_tab:
    ...
    work_type = st.radio(
        "처리할 업무를 선택하세요",
        ["파일·시트 합치기", "중복·누락·오류 찾기", "조건별 집계표", "부서별 명단 취합", "엑셀 VBA 코드 적용 매뉴얼", "AI 엑셀 활용 방법"],
        horizontal=True,
    )
    descriptions = {
    ...
    if work_type == "엑셀 VBA 코드 적용 매뉴얼":
        st.markdown("#### VBA를 처음 접하는 분을 위한 안내")
        st.write(
            "VBA는 Excel 안에서 반복 작업을 자동으로 실행시키는 명령문입니다. 예를 들어 매달 파일 20개를 열고, "
            "자료를 복사하고, 한 시트에 이어 붙이는 작업을 사람이 반복하는 대신 VBA 코드가 순서대로 처리할 수 있습니다."
        )
        v1, v2, v3 = st.columns(3)
        v1.markdown("**VBA 코드**  \nExcel이 따라 할 작업 순서를 글로 적은 명령문")
        v2.markdown("**매크로**  \n작성된 VBA 코드를 한 번에 실행하는 기능")
        v3.markdown("**AI의 역할**  \n업무 설명을 바탕으로 코드 초안을 작성하고 설명")
        st.info("쉽게 말하면, AI에게 작업 설명서를 주고 VBA 코드를 받은 다음, 그 코드를 Excel 안에 붙여 넣어 실행하는 방식입니다.")
        st.caption("VBA 자체가 AI는 아닙니다. AI는 코드 초안을 만들고, 실제 실행은 사용자의 PC에 설치된 데스크톱 Excel이 담당합니다.")
        learn_tab, prompt_tab, apply_tab, safety_tab = st.tabs(
            ["① 먼저 이해하기", "② AI에게 코드 요청", "③ Excel에 적용", "④ 오류·보안 확인"]
        )

        with learn_tab:
            st.markdown("#### 전체 과정은 네 단계입니다")
            st.markdown(
                """
1. **자동화할 작업을 글로 설명합니다.** 시트명, 열 이름, 결과 모습, 예외사항을 적습니다.
2. **허용된 AI에 설명을 입력해 VBA 코드를 받습니다.** 실제 개인정보 대신 열 이름과 가상 예시만 사용합니다.
3. **코드를 Excel의 모듈에 붙여 넣습니다.** 원본이 아닌 복사본에서 먼저 시험합니다.
4. **매크로를 실행하고 결과를 확인합니다.** 행 수, 합계, 누락 여부가 원본과 맞는지 비교합니다.
                """
            )
            st.markdown("#### 자주 보게 되는 용어")
            st.dataframe(
                pd.DataFrame([
                    ["통합문서", "Excel 파일 하나를 뜻합니다."],
                    ["워크시트", "파일 아래쪽의 '원본', '결과' 같은 시트 탭입니다."],
                    ["VBA 편집기", "코드를 입력하고 수정하는 Excel 내부 화면입니다. Alt + F11로 엽니다."],
                    ["모듈", "VBA 코드를 붙여 넣는 빈 문서 공간입니다."],
                    ["매크로", "Sub로 시작하는 코드 한 묶음을 실행하는 기능입니다."],
                    [".xlsm", "VBA 코드를 저장할 수 있는 Excel 파일 형식입니다."],
                ], columns=["용어", "뜻"]),
                use_container_width=True,
                hide_index=True,
            )
            st.markdown("#### 어떤 작업에 적합한가요?")
            ...
            st.markdown("#### 아주 짧은 코드 예시 읽어보기")
            st.code('''Sub ShowMessage()
    MsgBox "작업을 시작합니다."
End Sub''', language="vb")
            st.markdown(
                "- `Sub ShowMessage()`는 **ShowMessage라는 매크로의 시작**입니다.\n"
                "- `MsgBox`는 화면에 **안내창을 띄우라는 명령**입니다.\n"
                "- `End Sub`는 **매크로가 끝났다는 표시**입니다."
            )
        with prompt_tab:
            st.markdown("#### AI에게 VBA 코드 요청하기")
            st.write("아래 세 칸을 채우면 AI에 복사해 보낼 요청문이 자동으로 만들어집니다.")
            st.markdown(
                """
- **업무 설명:** 지금 사람이 어떤 순서로 작업하는지 적습니다.
- **시트명과 주요 열:** 실제 시트 탭 이름과 A열·B열에 무엇이 있는지 적습니다.
- **세부 조건과 예외:** 제목행, 빈칸, 기존 결과 시트 처리, 정렬 기준 등을 적습니다.
                """
            )
            task_examples = {
...
End Sub'''
        ...
        with apply_tab:
            st.markdown("#### 받은 VBA 코드를 Excel에 넣고 실행하기")
            st.warning("처음에는 실제 업무 원본이 아닌 복사본과 가상 데이터로 시험하세요.")
            st.markdown(
                """
**1단계 — 원본 파일 복사하기**

파일 탐색기에서 원본 파일을 복사해 `파일명_연습용.xlsx`처럼 새 이름을 붙입니다. VBA가 잘못 작동하면 여러 셀이 한꺼번에 바뀔 수 있으므로 원본에 바로 실행하지 않습니다.

**2단계 — 매크로 저장 형식으로 바꾸기**

연습용 파일을 Windows 데스크톱 Excel에서 엽니다. `파일 → 다른 이름으로 저장 → 파일 형식`에서 **Excel 매크로 사용 통합 문서(`.xlsm`)**를 선택합니다. `.xlsx`로 저장하면 VBA 코드가 사라질 수 있습니다.

**3단계 — VBA 편집기 열기**

키보드에서 `Alt + F11`을 누릅니다. 별도의 회색 창이 열리면 VBA 편집기입니다. 노트북에서 단축키가 작동하지 않으면 `Fn + Alt + F11`을 사용하거나 Excel의 `개발 도구 → Visual Basic`을 누릅니다.

**4단계 — 모듈 만들기**

왼쪽의 `VBAProject(현재파일명.xlsm)`를 클릭합니다. 위쪽 메뉴에서 `삽입(Insert) → 모듈(Module)`을 누릅니다. 왼쪽에 `Modules → Module1`이 생기고 오른쪽에 큰 빈 화면이 나타납니다.

**5단계 — 코드 붙여 넣기**

AI 답변에서 `Sub ...`로 시작하여 `End Sub`로 끝나는 코드 전체를 복사합니다. 설명 문장과 코드 상자 앞뒤의 ``` 표시는 제외하고 `Module1`의 빈 화면에 붙여 넣습니다.

**6단계 — 내 파일에 맞게 이름 확인하기**

코드의 `원본`, `결과`, `Sheet1` 같은 시트명이 실제 Excel 아래쪽 시트 탭 이름과 완전히 같은지 확인합니다. 띄어쓰기까지 같아야 합니다. 코드 상단에 열 번호나 폴더 경로를 수정하라는 안내가 있다면 내 파일 구조에 맞게 바꿉니다.

**7단계 — 코드 문법 검사하기**

VBA 편집기 위쪽 메뉴에서 `디버그(Debug) → VBAProject 컴파일(Compile VBAProject)`을 누릅니다. 오류창이 뜨면 실행하지 말고 표시된 줄과 오류 문구를 AI에 다시 알려 수정된 코드를 받습니다.

**8단계 — 매크로 실행하기**

`Alt + Q`로 Excel로 돌아갑니다. `Alt + F8`을 누르고 실행할 매크로 이름을 선택한 다음 `실행`을 누릅니다. 매크로 이름은 보통 코드 첫 줄의 `Sub 뒤에 적힌 이름`입니다.

**9단계 — 결과 검증하고 저장하기**

결과 시트의 행 수, 합계, 중복, 누락을 원본과 비교합니다. 결과가 맞을 때만 저장합니다. 예상과 다르면 저장하지 않고 파일을 닫은 뒤 연습용 복사본을 새로 만들어 다시 시험합니다.
                """
            )
            st.info("이 안내는 Windows 데스크톱 Excel 기준입니다. 웹용 Excel에서는 VBA를 만들거나 실행할 수 없습니다.")
            with st.expander("개발 도구 메뉴가 보이지 않을 때"):
                st.write(
                    "파일 → 옵션 → 리본 사용자 지정으로 이동한 뒤 오른쪽 목록에서 '개발 도구'를 체크하고 확인을 누르세요. "
                    "Alt + F11과 Alt + F8을 사용할 수 있다면 개발 도구 메뉴를 반드시 표시할 필요는 없습니다."
                )
            with st.expander("AI가 준 답변에서 무엇만 복사하나요?"):
                st.write(
                    "코드 상자 안에서 Option Explicit 또는 Sub 매크로이름()으로 시작해 End Sub로 끝나는 부분만 복사합니다. "
                    "AI의 설명 문장과 제목은 붙여 넣지 않습니다."
                )
...
- 기관의 매크로 실행·보안 정책이 우선이며, 차단된 설정을 임의로 해제하지 않습니다.
            ...
            st.markdown("#### 코드에서 이런 단어를 먼저 찾아보세요")
            st.dataframe(
                pd.DataFrame([
                    ["Kill", "파일을 삭제할 수 있습니다.", "실행 중단"],
                    ["Shell 또는 WScript.Shell", "다른 프로그램이나 명령을 실행할 수 있습니다.", "실행 중단"],
                    ["http, https, XMLHTTP", "인터넷으로 데이터를 보낼 수 있습니다.", "보안담당자 확인"],
                    ["SendMail, Outlook.Application", "이메일을 발송할 수 있습니다.", "보안담당자 확인"],
                    ["Clear, Delete, EntireRow.Delete", "셀 또는 행을 삭제할 수 있습니다.", "복사본에서 범위 확인"],
                    ["SaveAs, Save", "파일을 저장하거나 덮어쓸 수 있습니다.", "저장 경로와 파일명 확인"],
                ], columns=["코드에서 보이는 단어", "의미", "대응"]),
                use_container_width=True,
                hide_index=True,
            )
            ...
            st.dataframe(
                pd.DataFrame([
                    ["매크로 목록에 없음", "Alt + F8에 이름이 보이지 않음", "코드를 Module1에 넣었는지, 첫 줄이 Sub로 시작하는지 확인"],
                    ["첨자 사용이 잘못되었습니다", "지정한 시트를 찾지 못함", "코드의 시트명과 실제 시트 탭 이름을 띄어쓰기까지 똑같이 수정"],
                    ["형식이 일치하지 않습니다", "숫자·날짜 자리에 다른 형식이 있음", "해당 열의 문자, 빈칸, 오류값을 확인하고 AI에 예외 처리 요청"],
                    ["개체가 필요합니다", "통합문서·시트·범위 지정이 잘못됨", "오류가 난 줄과 파일 구조를 AI에 알려 코드 수정"],
                    ["매크로 실행이 차단됨", "기관 정책이나 보안 설정으로 실행 불가", "차단을 임의 해제하지 말고 정보보안 담당자에게 허용 여부 확인"],
                    ["저장 후 코드가 사라짐", ".xlsx 형식으로 저장함", ".xlsm 형식으로 다시 저장"],
                ], columns=["증상", "뜻", "해결 방법"]),
                use_container_width=True,
                hide_index=True,
            )
            st.markdown("#### 오류가 났을 때 AI에 다시 물어보는 문장")
            st.code(
                "아래 VBA 코드를 실행했더니 오류가 발생했습니다.\n"
                "오류 문구: [오류 문구 입력]\n"
                "노란색으로 표시된 줄: [해당 코드 한 줄 입력]\n"
                "실제 시트명과 열 구조: [예: 원본 시트, A열 접수번호, B열 부서]\n"
                "원본을 삭제하거나 덮어쓰지 않는 방식으로 코드를 수정하고, 오류 원인을 초보자도 이해할 수 있게 설명해 주세요.\n\n"
                "※ 실제 개인정보와 업무자료는 붙여 넣지 말고 열 이름과 가상 예시만 사용하세요.",
                language=None,
            )
